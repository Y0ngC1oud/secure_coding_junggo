...
> **macOS 사용자 주의**: macOS는 기본적으로 5000번 포트를 "AirPlay 수신 기능"이 사용 중이라 접속 시 403 에러가 날 수 있다. 이 경우 다른 포트로 실행하면 된다.
> ```bash
> PORT=5050 python run.py
> ```
> 그 후 `http://localhost:5050` 으로 접속한다. (또는 시스템 설정 > 일반 > AirDrop 및 Handoff에서 "AirPlay 수신 기능"을 꺼도 된다.)
...
`.env`에 `ADMIN_USERNAME`, `ADMIN_PASSWORD`를 채워두면, 서버를 시작할 때마다 해당 계정이 자동으로 생성/관리자 승격된다. 별도 명령 실행이 필요 없고, 계정이 이미 있으면 권한만 부여하고 비밀번호는 건드리지 않는다. 그 아이디/비밀번호로 바로 로그인하면 된다.

**방법 2 — CLI 명령**

1. 먼저 웹 화면에서 일반 회원가입을 진행한다.
2. 아래 명령에서 `<가입한 아이디>` 부분을 실제 아이디 문자열로 바꿔서(꺾쇠괄호 `<`, `>`는 입력하지 않음) 실행한다. **반드시 프로젝트 폴더로 이동해 venv를 켠 상태**에서 실행해야 한다.
